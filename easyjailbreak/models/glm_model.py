...
class GLMModel(WhiteBoxModelBase):
    """
...
def from_pretrained(model_name_or_path: str, model_name: str, tokenizer_name_or_path: Optional[str] = None,
                    dtype: Optional[torch.dtype] = None, **generation_config: Dict[str, Any]) -> GLMModel:
    """
    ...
    **Example**
    ...
    model = AutoModel.from_pretrained(model_name_or_path, trust_remote_code=True).half().cuda().eval()
    ...
    return GLMModel(model, tokenizer, model_name=model_name, generation_config=generation_config)