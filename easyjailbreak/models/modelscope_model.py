"""
This file contains a wrapper for Modelscope models, implementing various methods used in downstream tasks.
It includes the ModelscopeModel class that extends the functionality of the WhiteBoxModelBase class.
"""
...
class ModelscopeModel(WhiteBoxModelBase):
    """
    ModelscopeModel is a wrapper for Modelscope's transformers models.
    ...
    def __init__(
        ...
        """
        Initializes the ModelscopeModel with a specified model, tokenizer, and generation configuration.

        :param Any model: A Modelscope model.
        :param Any tokenizer: A Modelscope tokenizer.
    ...
    def generate(self, messages, input_field_name='input_ids', clear_old_history=True, **kwargs):
        ...
        output, history = self.model.chat(self.tokenizer, prompt, history=None)
        print(output)
    ...
    def __call__(self, *args, **kwargs):
        r"""
        Allows the ModelscopeModel instance to be called like a function, which internally calls the model's
        __call__ method.
...
def from_pretrained(model_name_or_path: str, model_name: str, tokenizer_name_or_path: Optional[str] = None,
                    dtype: Optional[torch.dtype] = None, **generation_config: Dict[str, Any]) -> ModelscopeModel:
    """
    ...
    :return ModelscopeModel: An instance of the ModelscopeModel class containing the imported model and tokenizer.
    ...
    **Example**
    ...
    model.generation_config = GenerationConfig.from_pretrained(model_name_or_path, trust_remote_code=True) # 可指定不同的生成长度、top_p等相关超参
    ...
    return ModelscopeModel(model, tokenizer, model_name=model_name, generation_config=generation_config)