...
class Qwen2Model(WhiteBoxModelBase):
    """
    ...
    def generate(self, messages, input_field_name='input_ids', clear_old_history=True, **kwargs):
        ...
        print("-------------------------------")
        model_inputs = self.tokenizer([text], return_tensors="pt").to(self.model.device.index)
...
def from_pretrained(model_name_or_path: str, model_name: str, tokenizer_name_or_path: Optional[str] = None,
                    dtype: Optional[torch.dtype] = None, **generation_config: Dict[str, Any]) -> Qwen2Model:
    """
    ...
    **Example**
    ...
    return Qwen2Model(model, tokenizer, model_name=model_name, generation_config=generation_config)